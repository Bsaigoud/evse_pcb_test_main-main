...
#endif
#if CONFIG_EXAMPLE_CONNECT_PPP && 1
...
{
#if CONFIG_EXAMPLE_CONNECT_ETHERNET && 1
...
#endif
#if CONFIG_EXAMPLE_CONNECT_PPP && 0
    example_ppp_shutdown();
    ESP_ERROR_CHECK(esp_unregister_shutdown_handler(&example_ppp_shutdown));
#endif
    return ESP_OK;
}
...
esp_err_t deinit_ethernet(void)
{
#if CONFIG_EXAMPLE_CONNECT_ETHERNET
    example_ethernet_shutdown();
    ESP_ERROR_CHECK(esp_unregister_shutdown_handler(&example_ethernet_shutdown));
